...
function validateEmails(input, output) {
  const inputFiles = (0, glob_1.sync)(input);
  if (!inputFiles.length) {
    console.log(
      chalk_1.default.red('Please provide a valid csv file as input'),
    );
    process.exit(1);
  }
  ...
  (0, __1.validation)(inputFiles, outputFile);
}
function analyseEmails(input, output) {
  ...
  const outputFile = path_1.default.resolve(output);
  (0, __1.analysis)(inputFiles, outputFile);
}
