...
exports.getUserDetails = (0, express_async_handler_1.default)(
  async (req, res) => {
...
const register = async (req, res) => {
  try {
    // Validate input data
    await (
      await (0, utils_1.validateUserSignUp)()
    ).validateAsync({
    ...
    const { firstName, lastName, email, password, phone_number, dob } =
      req.body;
    // Check if user already exists
    const userExists = await users_model_1.User.find({
    ...
    console.log(
      `User registered successfully: ${user.firstName} , Token : ${mytoken}`,
    );
  } catch (error) {
...
const login = async (req, res) => {
  ...
  const body = req.body;
  await (
    await (0, utils_1.validateUserSignIn)()
  ).validateAsync({
    email: body.email,
  ...
  if (
    user.length > 0 &&
    (await bcryptjs_1.default.compare(password, user[0].password))
  ) {
    const userInfo = await balances_model_1.Balance.findOne({
      userId: user[0]._id,
    });
